...
// Leemos el secreto y el tiempo de expiración del .env
// Si no existe en el .env, usa valores por defecto (7 días)
const JWT_SECRET = process.env.JWT_SECRET || process.env.jwtSecret || 'tu_secreto_jwt'
const JWT_EXPIRES = process.env.JWT_EXPIRES || '7d'

exports.register = async (req, res) => {
  const { nombre_usuario, contra_usuario, email_usuario } = req.body
  try {
    const existing = await db.query('SELECT 1 FROM usuarios WHERE email = $1', [email_usuario])
    ...
    // --- CORRECCIÓN AQUÍ: Usamos la variable JWT_EXPIRES ---
